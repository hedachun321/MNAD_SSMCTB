...
class SSMCTB(nn.Module):
    def __init__(self, channels, kernel_dim=1, dilation=1):
        ...
        self.conv4 = nn.Conv2d(in_channels=channels,
                               out_channels=channels,
                               kernel_size=kernel_dim)
        # self.conv5 = nn.Conv2d(in_channels=channels,
        #                        out_channels=channels,
        #                        kernel_size=kernel_dim)
        # self.conv6 = nn.Conv2d(in_channels=channels,
        #                        out_channels=channels,
        #                        kernel_size=kernel_dim)
        # self.conv7 = nn.Conv2d(in_channels=channels,
        #                        out_channels=channels,
        #                        kernel_size=kernel_dim)
        # self.conv8 = nn.Conv2d(in_channels=channels,
        #                        out_channels=channels,
        #                        kernel_size=kernel_dim)
    ...
    #     x = F.pad(x_in, (self.pad, self.pad, self.pad, self.pad), "constant", 0)
    #
    #     # 使用更大的切片范围覆盖中心区域周围的所有点
    #     x1 = self.conv1(x[:, :, :-self.border_input, :-self.border_input])
    #     x2 = self.conv2(x[:, :, self.border_input:, :-self.border_input])
    #     x3 = self.conv3(x[:, :, :-self.border_input, self.border_input:])
    #     x4 = self.conv4(x[:, :, self.border_input:, self.border_input:])
    #     x5 = self.conv5(x[:, :, :-self.border_input, self.border_input//2:-self.border_input//2])
    #     x6 = self.conv6(x[:, :, self.border_input:, self.border_input//2:-self.border_input//2])
    #     x7 = self.conv7(x[:, :, self.border_input//2:-self.border_input//2, :-self.border_input])
    #     x8 = self.conv8(x[:, :, self.border_input//2:-self.border_input//2, self.border_input:])
    #
    #     # 将所有卷积操作的结果相加，并经过 ReLU 激活函数
    #     x = self.relu(x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8)
    #
    #     x = self.transformer(x)
    #
    #     return x, torch.mean((x - x_in) ** 2)
...
if __name__ == '__main__':
    ...
    # 打印生成的张量
    print(tensor)
    ssmctb = SSMCTB(channels=1, kernel_dim=1, dilation=1)
    x, loss = ssmctb(tensor)
