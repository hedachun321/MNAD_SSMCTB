...
class Decoder(torch.nn.Module):
    def __init__(self, t_length = 5, n_channel =3):
        ...
        self.sspcab = SSPCAB(channels=128, kernel_dim=1, dilation=1, reduction_ratio=8)
        self.ssmctb = SSMCTB(channels=128, kernel_dim=1, dilation=1)
    ...
    def forward(self, x, skip1, skip2, skip3):
        ...
        # cat3_ = self.sspcab(cat3)
        # mse_loss = nn.MSELoss()
        # cost_sspcab = mse_loss(cat3, cat3_)
        tensorDeconv2 = self.moduleDeconv2(cat3)
        tensorDeconv2_, cost_ssmctb = self.ssmctb(tensorDeconv2)
        # tensorDeconv2_ = self.sspcab(tensorDeconv2)
        # mse_loss = nn.MSELoss()
        # cost_sspcab = mse_loss(tensorDeconv2, tensorDeconv2_)
        tensorUpsample2 = self.moduleUpsample2(tensorDeconv2_)
        cat2 = torch.cat((skip1, tensorUpsample2), dim = 1)
        
        output = self.moduleDeconv1(cat2)

                
        return output,cost_ssmctb

        # return output, cost_sspcab
...
class convAE(torch.nn.Module):
    ...
    def forward(self, x, keys,train=True):
        ...
        if train:
            ...
            output, cost_ssmctb = self.decoder(updated_fea, skip1, skip2, skip3)
            
            return output, fea, updated_fea, keys, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss, cost_ssmctb
        ...
        #test
            ...
            return output, fea, updated_fea, keys, softmax_score_query, softmax_score_memory, query, top1_keys, keys_ind, compactness_loss, cost_ssmctb
