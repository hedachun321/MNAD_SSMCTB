...
class Decoder(torch.nn.Module):
    def __init__(self, t_length = 5, n_channel =3):
        ...
        self.sspcab = SSPCAB(channels=128, dilation=1, reduction_ratio=8)
...
class convAE(torch.nn.Module):
    ...
    def forward(self, x, keys,train=True):
        ...
        if train:
            ...
            output, cost_sspcab = self.decoder(updated_fea, skip1, skip2, skip3)
            
            return output, fea, updated_fea, keys, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss,cost_sspcab
