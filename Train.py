...
parser.add_argument('--epochs', type=int, default=202, help='number of epochs for training')
...
parser.add_argument('--exp_dir', type=str, default='log_ssmctb_all', help='directory of log')
# parser.add_argument('--resume', type=str, default='exp/UR_Kinect/log/model_500.pth', help='file path of resume pth')
...
for epoch in range(args.epochs):
    ...
    for j,(imgs) in enumerate(train_batch):
        ...
        if args.method == 'pred':
            ...
            outputs, _, _, m_items, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss, cost_ssmctb = model.forward(imgs[:,0:12], m_items, True)
        ...
        loss = loss_pixel + args.loss_compact * compactness_loss + args.loss_separate * separateness_loss + 0.1 * cost_ssmctb
        loss.backward(retain_graph=True)
        optimizer.step()
        # 计算批次百分比
        # batch_percentage = (j + 1) / total_batches * 100
        # print(f'Epoch: {epoch + 1}/{args.epochs} Batch: {j + 1}/{total_batches} ({batch_percentage:.2f}%)', end='\r')
        pbar.set_postfix({
                        'Epoch': '{0} {1}'.format(epoch+1, args.exp_dir),
                        'loss': '{:.6f}'.format(loss.item()),
                        'compactness_loss': '{:.6f}'.format(compactness_loss.item()),
                        'separateness_loss': '{:.6f}'.format(separateness_loss.item()),
                        'cost_ssmctb': '{:.6f}'.format(cost_ssmctb.item()),
                        })
        pbar.update(1)
    ...
    print('Epoch:', epoch+1)
    if args.method == 'pred':
        print('Loss: Prediction {:.6f}/ Compactness {:.6f}/ Separateness {:.6f}/ cost_ssmctb {:.6f}'.format(loss_pixel.item(), compactness_loss.item(), separateness_loss.item(), cost_ssmctb.item()))
    else:
    ...
    pbar.close()
    if(epoch % 10 ==0):
        torch.save(model, os.path.join(log_dir, 'model_ssmctb_' + str(epoch) + '.pth'))
        torch.save(m_items, os.path.join(log_dir, 'keys_ssmctb_' + str(epoch) + '.pt'))

    
print('Training is finished')
# Save the model and the memory items
torch.save(model, os.path.join(log_dir, 'model_ssmctb_last.pth'))
torch.save(m_items, os.path.join(log_dir, 'keys_model_ssmctb_last.pt'))
