...
def anomaly_score_ssmctb(psnr, max_psnr, min_psnr):
    return ((psnr - min_psnr) / (max_psnr-min_psnr))
...
def anomaly_score_list_ssmctb(loss_ssmctb_list):
    anomaly_score_list = list()
    for i in range(len(loss_ssmctb_list)):
        anomaly_score_list.append(anomaly_score_ssmctb(loss_ssmctb_list[i], np.max(loss_ssmctb_list), np.min(loss_ssmctb_list)))
...
#     list_result = []
#     for i in range(len(list1)):
#         # list_result.append((alpha * list1[i] + (0.94 - alpha) * list2[i]) + 0.06 * list3[i])
#         list_result.append((alpha * list1[i] + (0.75 - alpha) * list2[i]) + 0.25 * list3[i])
#
#     return list_result

def score_sum_(list1, list2, list3, alpha, beta):
    list_result = []
    for i in range(len(list1)):
        # list_result.append((alpha * list1[i] + (0.94 - alpha) * list2[i]) + 0.06 * list3[i])
        list_result.append((alpha * list1[i] + (1 - beta - alpha) * list2[i]) + beta * list3[i])
