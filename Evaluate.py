...
parser.add_argument('--model_dir', type=str, default='./exp/ped2/pred/log_ssmctb_96.14/model_ssmctb_40.pth', help='directory of model')
parser.add_argument('--m_items_dir', type=str, default='./exp/ped2/pred/log_ssmctb_96.14/keys_ssmctb_40.pt', help='directory of model')
...
for k,(imgs) in enumerate(test_batch):
    ...
    if args.method == 'pred':
        outputs, feas, updated_feas, m_items_test, softmax_score_query, softmax_score_memory, _, _, _, compactness_loss, cost_ssmctb = model.forward(imgs[:,0:3*4], m_items_test, False)
...
for video in sorted(videos_list):
    ...
    # anomaly_score_total_list += score_sum(anomaly_score_list(psnr_list[video_name]),
    #                                  anomaly_score_list_inv(feature_distance_list[video_name]), anomaly_score_list_ssmctb(loss_ssmctb_list[video_name]), args.alpha)
